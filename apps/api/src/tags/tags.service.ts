...
export class TagsService {
  ...
  async findOne(id: string, userId: string) {
    const tag = await this.prisma.withConnection(() =>
  ...
  async getLeafTags(leafId: string, userId: string) {
    ...
    const leafTags = await this.prisma.withConnection(() =>
      this.prisma.leafTag.findMany({
        where: { leafId },
        include: { tag: true },
      }),
    );
